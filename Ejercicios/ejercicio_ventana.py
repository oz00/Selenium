...
class Test10ManejoVentanas(BaseTest02):
    ...
    def test_manejo_ventanas(self):
        ...
        btn_compartir_twitter = self.wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//*[@class='addthis_button_expanded']")))
        btn_compartir_twitter.click()
        btn_comp= self.wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//*[@class='at-icon at-icon-twitter'][1]")))
        btn_comp.click()
        time.sleep(5)

        #Switch a Ventana Twitter
        switch_window_by_title_contains(self.driver, "Twitter")#Moverme a otra ventana por medio del titulo
        print(self.driver.title) #Obtener el título de la ventana
        print(self.driver.current_window_handle)

        #Click al boton entrar (login) de la ventana Twitter
        btn_entrar = self.wait.until(expected_conditions.element_to_be_clickable((By.XPATH, "//*[text()='Regístrate']")))
        btn_entrar.click()
        time.sleep(5)
